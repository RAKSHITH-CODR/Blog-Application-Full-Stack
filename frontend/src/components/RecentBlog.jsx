...
const RecentBlog = () => {
    ...
    useEffect(() => {
        const getAllPublishedBlogs = async () => {
            try {
                const res = await axios.get(`https://blog-application-full-stack.onrender.com/api/v1/blog/get-published-blogs`,
  ...
  return (
    <div className='bg-gray-100 dark:bg-gray-800 pb-10'>
        ...
        <div className='max-w-7xl mx-auto flex gap-6'>
            ...
            <div className="bg-white hidden md:block dark:bg-gray-700 w-[350px] p-5 rounded-2xl mt-10 border">
            <h1 className='text-2xl font-semibold'>Popular Categories</h1>
            <div className="my-5 flex flex-wrap gap-3 mb-10">
                {
                    ["Blogging", "Web Development", "Cooking", "Travel", "Sports", "Sustainability", "Gaming", "Fitness", "Education"].map((item, index) => {
                        return <Badge onClick={()=>navigate(`/search?q=${item}`)} className="cursor-pointer" key={index}>{item}</Badge>
                    })
                }
            </div>
            <h1 className='text-xl font-semibold mb-2'>Subscribe to Newsletter</h1>
            ...
            <div className="mt-7">
                <h2 className='text-xl font-semibold my-3'>Suggested Blogs</h2>
                <ul>
                    {
